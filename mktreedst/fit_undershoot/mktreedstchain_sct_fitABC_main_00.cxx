...
// sprintf(s_out,"dst9_%04d-%04d.root",runs[ind_rmin],runs[ind_rmax]);
 ...
 sprintf(s_out,"dst9_m%02d_%04d-%04d.root",MOD,runs[ind_rmin],runs[ind_rmax]);
}
}
...
if(tail<989 || tail > 999) continue; //mod 0 take stopped in PD1 only
printf("ibeg %d %f\n",ibeg,tail);
...
//   }
  ...
  fitter->SetParameter(6,"t1",80,.1,50.,10000);//Tz zero crossing time relative to dx
...
//if(mod==30&& isig==1)fitter->SetParameter(7,"t2",t2/2.,.01,20, 100);//T1 -> decay1
  ...
  fitter->SetParameter(8,"t3",t3,.01,150, 700);//T3 -> decay2
//  fitter->FixParameter(8);//T3 -> decay2

  fitter->SetParameter(9,"t9",t9,.01,.1, 40);//tr -> common si rise time
...
//  fitter->SetParameter(6,"t1",30,.001,.01,10000);//Tz zero crossing time relative to dx
